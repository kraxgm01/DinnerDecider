...
final class RunSuspend implements Continuation<Unit> {
    ...
    public final Result<Unit> m1216getResultxLWZpok() {
        return this.result;
    }
    ...
    public void resumeWith(Object result2) {
        synchronized (this) {
            setResult(Result.m28boximpl(result2));
            notifyAll();
    ...
    public final void await() {
        synchronized (this) {
            while (true) {
                Result result2 = m1216getResultxLWZpok();
                if (result2 == null) {
                    wait();
                } else {
                    ResultKt.throwOnFailure(result2.m38unboximpl());
                }
            }
        }
    }
